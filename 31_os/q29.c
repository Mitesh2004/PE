...
void h(int s){
    ...
    else { puts("Child got SIGQUIT. My DADDY killed me!!!"); exit(0); }
}
...
int main(){
    ...
    if(!pid){  // child
        ...
        sleep(1);
        for(int i=0;i<10;i++){ kill(pid,i%2?SIGINT:SIGHUP); sleep(3); }
        kill(pid,SIGQUIT);
        wait(NULL);
...
/*
m@Debian:~/Desktop/OS/MCS_Sem-1/golfed_code$ nano q29.c
m@Debian:~/Desktop/OS/MCS_Sem-1/golfed_code$ gcc q29.c
...
Child got SIGHUP
Child got SIGINT
Child got SIGQUIT. My DADDY killed me!!!
...
void sigquit_handler(int sig) {
    printf("Child received SIGQUIT. My DADDY has Killed me!!!\n");
    exit(0);
}
...
int main() {
    ...
    if (pid == 0) {
        ...
        for (int i = 0; i < 10; i++) { // 10 iterations * 3 sec = 30 sec
            if (i % 2 == 0)
                kill(pid, SIGHUP);
            else
                kill(pid, SIGINT);
